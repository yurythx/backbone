...
export function GlobalSearch() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            const res = await api.get(`/api/core/search/?q=${encodeURIComponent(query)}`)
            return res.data
        },
    ...
    const onSelect = (type: string, id: number | string, item?: any) => {
        setOpen(false)
        setQuery("")
        if (type === 'article') router.push(`/artigos?id=${id}`)
        if (type === 'page') router.push(`/cms?id=${id}`)
        if (type === 'message') router.push(`/messenger?conversation=${item.conversation}&message_id=${id}`)
