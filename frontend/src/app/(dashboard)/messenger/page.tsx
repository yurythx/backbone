...
export default function MessengerPage() {
  ...
  const advancedFilteredResults = filteredResults.filter((m) => {
    ...
    const byUser = userFilter.trim().length === 0
      ? true
      : (m.sender_username || "").toLowerCase().includes(userFilter.trim().toLowerCase())
    ...
    const byReactions = onlyWithReactions ? Array.isArray(m.reactions) && m.reactions.length > 0 : true
  ...
  const openMessage = async (msg: Message) => {
    try {
      ...
      // conv.participants is a list of IDs (numbers) — standard DRF ManyToMany serialization.
      ...
      if (otherId !== undefined) {
        ...
        // Fetch the contact's full profile (avatar, username, etc.)
  ...
  // Sincroniza contato com conversationId da URL
  useEffect(() => {
    ...
    const fetchConversation = async () => {
      try {
        ...
        if (conversation.is_group) {
          ...
          // participants pode ser lista de IDs ou objetos dependendo do serializer
  ...
  return (
    <div className="flex h-[calc(100vh-theme(spacing.32))] border border-border/50 rounded-2xl overflow-hidden bg-background/50 backdrop-blur-sm shadow-sm flex-col">
        ...
        {debounced.length >= 3 && (
          <div className="mt-2 max-h-64 overflow-auto border rounded-xl bg-background/90 backdrop-blur-sm">
            ...
            {advancedFilteredResults.slice(0, 20).map((m) => (
              <div key={`${m.conversation}-${m.id}`} className="p-3 border-b last:border-b-0 flex items-center gap-3">
                <Badge variant="outline" className="text-[10px] px-2">#{m.conversation}</Badge>
                <div className="flex-1 min-w-0">
                  <p className="text-sm truncate">{m.content || m.file_name || 'Mensagem'}</p>
                  <p className="text-[10px] text-muted-foreground">{new Date(m.created_at).toLocaleString()}</p>
                </div>
                <Button size="sm" variant="outline" onClick={() => openMessage(m)}>Abrir</Button>
              </div>
            ))}
          </div>
        )}
