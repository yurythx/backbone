...
def run():
    ...
    admin_user = User.all_objects.filter(is_superuser=True, company=company).first() or User.all_objects.filter(company=company).first()
    if not admin_user:
    ...
    pipeline, created = Pipeline.all_objects.get_or_create(
        company=company,
    ...
    tickets = [
        ...
        ("Servidor de Arquivos Offline", "Urgente: Unidade Z: inacessível para todos os departamentos.", "URGENT", "Planejados"),
