...
# 1. Create Company
company, _ = Company.objects.get_or_create(slug="suporte", defaults={"name": "Suporte Backbone", "is_active": True})
...
modules_data = [
    ...
    {"code": "pages", "name": "CMS (Páginas)", "description": "Gestão de páginas institucionais", "is_default": True},
    {
        "code": "finance",
        "name": "Financeiro",
        "description": "Controles financeiros e fluxo de caixa",
        "is_default": False,
    },
...
for m_data in modules_data:
    module, created = Module.objects.get_or_create(
        code=m_data["code"],
        defaults={"name": m_data["name"], "description": m_data["description"], "is_default": m_data["is_default"]},
    )
    if created:
...
if not User.objects.filter(username="admin").exists():
    try:
        user = User.objects.create_superuser(
            username="admin", email="admin@localhost", password="password123", company=company
        )
