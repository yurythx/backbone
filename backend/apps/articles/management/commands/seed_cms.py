...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for company in companies:
            ...
            for cat_data in categories_data:
                try:
                    cat, created = Category.all_objects.get_or_create(
                        company=company, slug=cat_data["slug"], defaults={"name": cat_data["name"]}
                    )
                    created_categories.append(cat)
                    if created:
                        self.stdout.write(self.style.SUCCESS(f"  [+] Created Category: {cat.name}"))
                except Exception as e:
                    self.stdout.write(self.style.WARNING(f"  [!] Failed to process Category {cat_data['name']}: {e!s}"))
            ...
            for tag_name in tags_data:
                try:
                    tag, created = Tag.all_objects.get_or_create(
                        company=company, slug=slugify(tag_name), defaults={"name": tag_name}
                    )
                    created_tags.append(tag)
                    if created:
                        self.stdout.write(self.style.SUCCESS(f"  [+] Created Tag: {tag.name}"))
                except Exception as e:
                    self.stdout.write(self.style.WARNING(f"  [!] Failed to process Tag {tag_name}: {e!s}"))
            ...
            for _idx, article_data in enumerate(articles_data):
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(f"  [!] Failed to process Article {article_data['title']}: {e!s}")
                    )
