...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        features_data = [
            ...
            {
                "code": "storage_limit_mb",
                "name": "Espaço em Disco (MB)",
                "description": "Limite de armazenamento de mídia",
            },
            ...
            {"code": "api_access", "name": "Acesso a API Pública", "description": "Webhooks e API Keys"},
            {
                ...
                "description": "Dashboard de Insights e Engajamento",
            },
        ]
        ...
        for p in plans_data:
            ...
            for f_code, f_value in p["features"].items():
                PlanFeature.objects.update_or_create(
                    plan=plan, feature=features_objs[f_code], defaults={"value": f_value}
                )
    ...
    def verify_seed_health(self):
        ...
        if plan_feature_count == 0:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"  ✓ Plan-Feature mappings: {plan_feature_count}/{expected_plan_features}")
            )
