...
from apps.messenger.models import Message, Conversation
...
class GlobalSearchViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        # 2. Pages
        pages = Page.objects.filter(company=company).filter(
        ...
        contacts = User.objects.filter(company=company).filter(
