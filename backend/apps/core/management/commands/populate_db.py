...
from apps.licensing.models import License, Plan
        ...
        with transaction.atomic():
            ...
            for i in range(5):
                ...
                company = Company.objects.create(
                    name=company_name, slug=company_slug, domain=self.fake.domain_name() + str(i)
                )
    ...
    def populate_company(self, company):
        ...
        if users_needed > 0:
            for _ in range(users_needed):
                ...
                User.objects.create_user(
                    username=username, email=f"{username}@{company.domain}", password="password123", company=company
                )
        ...
        if Category.objects.filter(company=company).count() < 20:
            for _ in range(20):
                ...
                Category.objects.create(company=company, name=name, slug=slugify(name))
        ...
        if Conversation.objects.filter(company=company).count() < 20:
            for _ in range(20):
                c = Conversation.objects.create(company=company, title=self.fake.bs().capitalize(), is_group=True)
                ...
                for _ in range(20):
                    Message.objects.create(
                        company=company, conversation=c, sender=random.choice(parts), content=self.fake.sentence()
                    )
