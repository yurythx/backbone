    ...
    def handle(self, *args, **options):
        ...
        for config in configs:
            ...
            if config.smtp_password and not config.smtp_password_encrypted:
                self.stdout.write(
                    self.style.WARNING(f'Migrating password for: {config.company.name}')
                )
                ...
                skip_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'Skipping {config.company.name}: already encrypted')
                )
            else:
                skip_count += 1
                self.stdout.write(
                    self.style.WARNING(f'Skipping {config.company.name}: no password set')
                )
