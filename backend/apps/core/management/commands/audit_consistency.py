...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        inconsistent_articles = Article.objects.filter(
            is_public=True
        ).exclude(status='published')

        if inconsistent_articles.exists():
            self.stdout.write(self.style.WARNING(f"Artigos: Encontrados {inconsistent_articles.count()} itens inconsistentes!"))
            for art in inconsistent_articles:
                ...
                if fix:
                    ...
                    self.stdout.write(self.style.SUCCESS(f"    [FIX] Visibilidade pública removida."))
        else:
