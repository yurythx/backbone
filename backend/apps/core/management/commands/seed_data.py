...
from apps.licensing.models import Feature, License, Plan
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # 1. Create Companies
        blackbone, _ = Company.objects.get_or_create(name="BlackBone HQ", slug="blackbone", domain="blackbone.com")
        ironminds, _ = Company.objects.get_or_create(name="IronMinds Ltd", slug="ironminds", domain="ironminds.com")
        rootco, _ = Company.objects.get_or_create(name="Empresa Raiz", slug="raiz", domain="raiz.local")
        ...
        feature_chat, _ = Feature.objects.get_or_create(
            name="Messenger", code="messenger", description="Real-time chat"
        )
        ...
        if not User.all_objects.filter(username="admin_blackbone").exists():
            User.objects.create_superuser(
                username="admin_blackbone", email="admin@blackbone.com", password="password123", company=blackbone
            )
        ...
        if not User.all_objects.filter(username="suporte").exists():
            User.all_objects.create_superuser(
                username="suporte", email="suporte@raiz.local", password="suporte123", company=rootco
            )
        ...
        if not User.all_objects.filter(username="colab_raiz").exists():
            User.all_objects.create_user(
                username="colab_raiz", email="colab@raiz.local", password="password123", company=rootco
            )

        self.stdout.write(
            self.style.SUCCESS(
                "Users created: admin@blackbone.com, admin@ironminds.com (pass: password123), suporte@raiz.local (pass: suporte123)"
            )
        )
