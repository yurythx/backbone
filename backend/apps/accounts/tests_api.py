...
class AccountsAPITest(APITestCase):
    ...
    def test_invitation_cross_tenant_isolation(self):
        ...
        self.assertNotIn("hacker@tenantb.com", emails_returned,
                         "Cross-tenant: convite de outro tenant não deve aparecer")
    ...
    def test_non_admin_cannot_create_users(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN,
                         "Usuário sem permissão não deve conseguir criar usuários")
    ...
    def test_accept_invitation_duplicate_email_two_tenants(self):
        ...
        inv_a = Invitation.objects.create(
            company=self.company, email="dup@test.com",
            role=role_a, status='pending'
        )

        # Convite do tenant B com o mesmo email
        inv_b = Invitation.objects.create(
            company=company_b, email="dup@test.com",
            role=role_b, status='pending'
        )

        self.client.logout()

        # Aceitar o primeiro convite
        res_a = self.client.post('/api/accounts/invitations/accept/', {
            "token": inv_a.token,
            "first_name": "Dup",
            "last_name": "User",
            "password": "securepass",
        ...
        self.assertIn(res_a.status_code, (status.HTTP_200_OK, status.HTTP_201_CREATED),
                      f"Primeiro aceite falhou: {res_a.data}")

        # Aceitar o segundo convite com o mesmo email — não deve dar IntegrityError
        res_b = self.client.post('/api/accounts/invitations/accept/', {
            "token": inv_b.token,
            "first_name": "Dup",
            "last_name": "User",
            "password": "securepass",
        ...
        self.assertNotEqual(user_a.username, user_b.username,
                            "Usernames devem ser únicos mesmo com o mesmo email")
